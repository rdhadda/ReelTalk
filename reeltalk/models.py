...
class Movie(db.Model):
    ...
    reviews = db.relationship(
        'Review', backref='movie', lazy=True, cascade="all, delete"
    )

    def __repr__(self):
        return (
            f"<Movie id={self.id} | tmdb_id={self.tmdb_id} | "
...
class Review(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    # ondelete="CASCADE" added. If either a User or Movie is removed
    # from the db, then all corresponding reviews are also removed
    ...
    def __repr__(self):
        return (f"<Review id={self.id}| user_id={self.user_id}| "
                f"movie_id={self.movie_id}| created_at={self.created_at}>")
